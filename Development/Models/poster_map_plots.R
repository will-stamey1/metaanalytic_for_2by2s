...
map_comp_plot <- function(lnRRs, logitp1s, n, lnRRsd = 1, iter = 1, 
  ...
  if(length(lnRRs) != length(logitp1s)){stop("Vectors must be of equal length.")}
  ...
  if(return_samps & ! return_counts){
    ...
    output <- list(output, samps, list("n1" = n1, "n11" = n11))
  }
  return(output)  
}
...
#a <- map_comp_plot(lnRRs, logitp1s, 30, lnRRsd = 0.1, return_samps = T)
...
nexp <- 10
lnRRs <- rnorm(nexp, mean = -0.3, sd = 1)
logitp1s <- rnorm(nexp, -1, 1)


#empp1 <- rbinom(nexp, n, prob = p1s)/n
...
for(sd in RRsds){
  for(N in Ns){
    out <- map_comp_plot(lnRRs = lnRRs, logitp1s = logitp1s, n = N, 
                         lnRRsd = sd, return_samps = T)
