...
hier2b2 <- function(iter = 1, nexp = 10, n = 100, p1_alpha = 15, p1_beta = 30, lnRR_mu = 0, 
                    lnRR_sig2 = 0.5, prior_p1mu_alpha = 1, prior_p1mu_beta = 1, 
  ...
  output <- data.frame(matrix(NA, iter, 14))
  ...
  for(i in 1:iter){
    ...
    output[i,]$lnRRmu_covr <- stuff$quantiles["lnRR_mu",5] > exp(lnRR_mu) & stuff$quantiles["lnRR_mu",1] < exp(lnRR_mu)
